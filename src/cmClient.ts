...
export class ClouderaManagerClient {
  ...
  async listClusterCommands(
    clusterName: string,
    opts?: { view?: "summary" | "full"; limit?: number; offset?: number }
  ): Promise<any> {
    if (!clusterName) throw new Error("clusterName is required");
    ...
    const res = await this.request(`/clusters/${encodeURIComponent(clusterName)}/commands${q}`, { method: "GET" });
    await this.assertOk(res, "listClusterCommands");
    return res.json();
  }

  async listServiceCommands(
    clusterName: string,
    serviceName: string,
    opts?: { view?: "summary" | "full"; limit?: number; offset?: number }
  ): Promise<any> {
    ...
    const res = await this.request(
      `/clusters/${encodeURIComponent(clusterName)}/services/${encodeURIComponent(serviceName)}/commands${q}`,
      { method: "GET" }
    );
    await this.assertOk(res, "listServiceCommands");
    return res.json();
  }

  async inspectHosts(hostIds?: string[]): Promise<any> {
    const body = hostIds && hostIds.length > 0 ? { hostIds } : {};
    const res = await this.request(`/cm/commands/inspectHosts`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
    await this.assertOk(res, "inspectHosts");
    return res.json();
  }

  async listParcels(clusterName: string, view?: "summary" | "full"): Promise<any> {
    if (!clusterName) throw new Error("clusterName is required");
    const q = view ? `?view=${encodeURIComponent(view)}` : "";
    const res = await this.request(`/clusters/${encodeURIComponent(clusterName)}/parcels${q}`, { method: "GET" });
    await this.assertOk(res, "listParcels");
    return res.json();
  }

  async getParcelsUsage(clusterName: string): Promise<any> {
    if (!clusterName) throw new Error("clusterName is required");
    const res = await this.request(`/clusters/${encodeURIComponent(clusterName)}/parcels/usage`, { method: "GET" });
    await this.assertOk(res, "getParcelsUsage");
    return res.json();
  }
